...
public class GameProcess {
    ...
    public static boolean nextCoordFire(int prevCoordX, int prevCoordY, int vectorFire) {
        boolean trueCoordFireNext = false;

        if (vectorFire == 0) {
            if (prevCoordY - 1 > 0) trueCoordFireNext = true;
            else trueCoordFireNext = false;
        }
        if (vectorFire == 1) {
            if (prevCoordX + 1 < 11) trueCoordFireNext = true;
            else trueCoordFireNext = false;
        }
        if (vectorFire == 2) {
            if (prevCoordY + 1 < 11) trueCoordFireNext = true;
            else trueCoordFireNext = false;
        }
        if (vectorFire == 3) {
            if (prevCoordX - 1 > 0) trueCoordFireNext = true;
            else trueCoordFireNext = false;
        }
